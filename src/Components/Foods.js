...
export default function Foods({foods, setFood}) {
        ...
        <>
            <ul>
                {foodsData.map((food, index) => (
                    <Product key={index} image={food.image} name={food.name} description={food.description} price={food.price} chosen={foods} func={setFood}/>
                ))}
            </ul>
        </>
    );
